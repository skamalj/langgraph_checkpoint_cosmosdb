"""
E2E: CosmosDBSaver + MessageReducer.on_prune -> LangGraph BaseStore.

Proves the chain  checkpointer.put -> reducer -> on_prune hook -> store.put
with a real compiled graph, a real Cosmos DB container for the checkpoints, and a
...
Requires COSMOSDB_ENDPOINT / COSMOSDB_KEY (Cosmos DB account with a "langgraph" database).
...
DATABASE = os.environ.get("COSMOS_TEST_DB", "langgraph")
CONTAINER = os.environ.get("COSMOS_TEST_CONTAINER", "checkpointtest")
MIN_MESSAGES = 4
MAX_MESSAGES = 6
...
def _build(store: BaseStore, hook):
    ...
    saver = CosmosDBSaver(database_name=DATABASE, container_name=CONTAINER, reducer=reducer)
    g = StateGraph(State)
...
def _cleanup(saver, config):
    delete = getattr(saver, "delete", None)
    if callable(delete):
        delete(config)
